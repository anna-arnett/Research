...
from library.BNN_module import *
from library.Quantization_module import *
...
class local_planar_guidance(nn.Module):
    ...
    def forward(self, plane_eq, focal):
        ...
        u = self.u.repeat(plane_eq.size(0), plane_eq.size(2) * int(self.upratio), plane_eq.size(3)).to(self.device)
        u = (u - (self.upratio - 1) * 0.5) / self.upratio
        
        v = self.v.repeat(plane_eq.size(0), plane_eq.size(2), plane_eq.size(3) * int(self.upratio)).to(self.device)
...
class Decoder(nn.Module):
    def __init__(self, max_depth, feat_out_channels, num_features=512):
        ...
        # BAYESIAN LAYERS
        #self.reduc1x1 = BNN_LINEAR(num_features // 16, num_features // 32)
        #self.conv1 = torch.nn.Sequential(BNN_LINEAR(num_features // 16 + 4, num_features // 16),
        #                                 nn.ELU())
        #self.get_depth = torch.nn.Sequential(BNN_LINEAR(num_features // 16, 1),
        #                                     nn.Sigmoid())
        ...
        # new
        #sample_tensor = torch.rand(1, num_features // 16, 4, 4)
        #flattened_size = sample_tensor.view(1, -1).size(1)
        #self.fc_input_size = (num_features // 16) * 4 * 4
        self.fc = nn.Linear(num_features // 16 * 4 * 4, 1000)
    ...
    def forward(self, features, focal):
        ...
        # new
        #flat = iconv1.view(iconv1.size(0), -1)
        #fc_output = self.fc(flat)
        #fc_output = fc_output.view(final_depth.size())
        #final_depth = self.max_depth * (self.get_depth(iconv1) + fc_output.view(fc_output.size(0), 1, 1, 1))
        
        return depth_8x8_scaled, depth_4x4_scaled, depth_2x2_scaled, reduc1x1, final_depth#, fc_output